...
class EventLoopThread(threading.Thread):
    ...
    def run(self):
        log('Starting event loop on background thread')
        AppHelper.runConsoleEventLoop(installInterrupt=True)

    def stop(self):
        log('Stop the event loop')
...
class PeripheralDelegate(NSObject):
    def init(self):
        ...
        dispatch_queue = libdispatch.dispatch_queue_create(b'peripheral_queue', None)
        self.manager = CBPeripheralManager.alloc().initWithDelegate_queue_(self, dispatch_queue)
...
def main():
    ...
    time.sleep(1)  # Give time for event loop to start
    ...
    # Start central in another thread
